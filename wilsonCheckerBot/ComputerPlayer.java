...
/**
 * <p>Title: Computer Player</p>
 * <p>Description: AI that plays checkers given a board state.</p>
 * @author Joshua Wilson
 * @version 1.0
 */

public class ComputerPlayer extends java.rmi.server.UnicastRemoteObject implements referee.Player {
	private String name;
	...
	public ComputerPlayer(String Name) throws java.rmi.RemoteException {
		//this.name = name;
	...
	//complete the alpha/beta prune and figure out the actual int[] to return to the referee
	...
	//Generate a heuristic for a given board
	...
	//Generate reachable boards given a board state
	public List<Board> ReachableBoards(Board b, Player p){
		...
		//check non double cap boards for kings
	...
	public int BoardValue(Board initBrd, Player p, int ply, int alpha, int beta, Board theBest) throws java.rmi.RemoteException {
		...
		if(p == this){
			pIsWhite = isWhite;
			notThisPlayer = new ComputerPlayer("temp");
		} else {
			pIsWhite = !isWhite;
			notThisPlayer = this;
		}
	...
	public static void main (String[] args) {
		if (args.length != 2 || (!args[0].equals("1") && !args[0].equals("2"))) {
      		System.err.println("Usage: java ComputerPlayer X FOO, where X is 1 for registering the agent as 'first',\n"+
    	...
    	//name doesn't actually work for some reason
    	try {
     		ComputerPlayer p = new ComputerPlayer(playerName);
     		if(args[0].equals("1")){
    			p.isWhite = false;
    		} else {
    			p.isWhite = true;
	...
	//print out the board for debugging purposes. Copied from HumanPlayer
